#!/usr/bin/env bash
# run_eval_pi0fast_libero.sh — Evaluate lerobot/pi0fast-libero (libero_spatial only)
#
# Usage:
#   bash run_eval_pi0fast_libero.sh
#   bash run_eval_pi0fast_libero.sh --n-episodes 10 --batch-size 5
set -euo pipefail
...
JOB_NAME="pi0fast-libero-spatial"
POLICY_PATH="lerobot/pi0fast-libero"
OUTPUT_BASE="pi0fast-eval-output"
LOG_BASE="pi0fast-eval-logs"
...
echo "=== pi0fast-libero: Single-suite eval launcher ==="
...
BATCH_SCRIPT=$(mktemp /tmp/pi0fast_eval.XXXX.sh)
...
echo ""
echo "=== Job submitted for pi0fast-libero ==="
