#!/usr/bin/env bash
# run_eval_pi05_libero_v2.sh — Evaluate lerobot/pi05-libero across all LIBERO suites (v2 venv)
#
# Usage:
#   bash run_eval_pi05_libero_v2.sh                                      # 50 episodes per suite
#   bash run_eval_pi05_libero_v2.sh --n-episodes 10 --batch-size 5       # quick test
#   bash run_eval_pi05_libero_v2.sh --partition unkillable --timeout 4:00:00
set -euo pipefail
...
JOB_NAMES=(pi05-v2-spatial pi05-v2-object pi05-v2-goal pi05-v2-10 pi05-v2-90)
POLICY_PATH="lerobot/pi05-libero"
POLICY_TAG="pi05-v2"
OUTPUT_BASE="pi05-eval-output-v2"
LOG_BASE="pi05-eval-logs-v2"
...
echo "=== pi05-libero v2: Multi-suite eval launcher ==="
...
for i in "${!TASK_SUITES[@]}"; do
    ...
    BATCH_SCRIPT=$(mktemp /tmp/pi05_v2_eval_${SUITE}.XXXX.sh)
...
SCRIPTEOF
    ...
    sed -i \
        ...
        -e "s|OUTPUT_DIR_PLACEHOLDER|${OUTPUT_DIR}/${SUITE}|g" \
        "$BATCH_SCRIPT"

    echo "--- Submitting ${JOB_NAME} (${SUITE}) ---"
...
echo ""
echo "=== All jobs submitted for pi05-libero v2 ==="
