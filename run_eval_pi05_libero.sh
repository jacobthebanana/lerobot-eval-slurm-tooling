#!/usr/bin/env bash
# run_eval_pi05_libero.sh — Evaluate lerobot/pi05-libero across all LIBERO suites
#
# Usage:
#   bash run_eval_pi05_libero.sh                                      # 50 episodes per suite
#   bash run_eval_pi05_libero.sh --n-episodes 10 --batch-size 5       # quick test
#   bash run_eval_pi05_libero.sh --partition unkillable --timeout 4:00:00
set -euo pipefail
...
JOB_NAMES=(pi05-spatial pi05-object pi05-goal pi05-10 pi05-90)
POLICY_PATH="lerobot/pi05-libero"
POLICY_TAG="pi05"
OUTPUT_BASE="pi05-eval-output"
LOG_BASE="pi05-eval-logs"
...
echo "=== pi05-libero: Multi-suite eval launcher ==="
...
for i in "${!TASK_SUITES[@]}"; do
    ...
    BATCH_SCRIPT=$(mktemp /tmp/pi05_eval_${SUITE}.XXXX.sh)
...
echo ""
echo "=== All jobs submitted for pi05-libero ==="
