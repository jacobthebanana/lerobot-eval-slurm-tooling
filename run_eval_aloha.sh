#!/usr/bin/env bash
# run_eval_aloha.sh — Submit closed-loop ALOHA eval using ACT aloha
#
# Usage:
#   bash run_eval_aloha.sh                                   # 50 episodes
#   bash run_eval_aloha.sh --n-episodes 10 --batch-size 5    # quick test
#   bash run_eval_aloha.sh --task "AlohaTransferCube-v0"     # different task
set -euo pipefail
...
MODEL_DIR="${SCRATCH}/local-models/act_aloha_sim_transfer_cube_human"
OUTPUT_DIR="${SCRATCH}/xvla-eval-output/$(date +%Y%m%d_%H%M%S)_aloha"

mkdir -p "${SCRATCH}/xvla-eval-logs"

BATCH_SCRIPT=$(mktemp /tmp/aloha_eval_batch.XXXX.sh)
trap 'rm -f "$BATCH_SCRIPT"' EXIT

cat > "$BATCH_SCRIPT" << 'SCRIPTEOF'
#!/usr/bin/env bash
#SBATCH --job-name=act-aloha-eval
#SBATCH --output=${SCRATCH}/xvla-eval-logs/act-aloha-eval-%j.out
#SBATCH --error=${SCRATCH}/xvla-eval-logs/act-aloha-eval-%j.err
...
echo "  Model:       act_aloha_sim_transfer_cube_human (local)"
echo "  Env type:    aloha"
echo "  Task:        ${TASK}"
