#!/usr/bin/env bash
# run_eval_vla_jepa_libero.sh — Evaluate lerobot/VLA-JEPA-LIBERO across all LIBERO suites
#
# Usage:
#   bash run_eval_vla_jepa_libero.sh                                      # 50 episodes per suite
#   bash run_eval_vla_jepa_libero.sh --n-episodes 10 --batch-size 5       # quick test
#   bash run_eval_vla_jepa_libero.sh --partition unkillable --timeout 4:00:00
set -euo pipefail
...
JOB_NAMES=(vlajepa-spatial vlajepa-object vlajepa-goal vlajepa-10 vlajepa-90)
POLICY_PATH="lerobot/VLA-JEPA-LIBERO"
POLICY_TAG="vlajepa"
OUTPUT_BASE="vlajepa-eval-output"
LOG_BASE="vlajepa-eval-logs"
...
echo "=== VLA-JEPA-LIBERO: Multi-suite eval launcher ==="
...
for i in "${!TASK_SUITES[@]}"; do
    ...
    BATCH_SCRIPT=$(mktemp /tmp/vlajepa_eval_${SUITE}.XXXX.sh)
...
echo ""
echo "=== All jobs submitted for VLA-JEPA-LIBERO ==="
