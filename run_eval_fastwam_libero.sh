#!/usr/bin/env bash
# run_eval_fastwam_libero.sh — Submit closed-loop LIBERO eval using FastWAM
#
# Usage:
#   bash run_eval_fastwam_libero.sh                                    # libero_spatial (default)
#   bash run_eval_fastwam_libero.sh --task libero_10                   # single suite
#   bash run_eval_fastwam_libero.sh --n-episodes 10 --batch-size 5     # quick test
set -euo pipefail
...
POLICY_PATH="lerobot/fastwam_libero_uncond_2cam224"
SCRATCH="${SCRATCH}"
OUTPUT_BASE="${SCRATCH}/fastwam-eval-output"
LOG_DIR="${SCRATCH}/fastwam-eval-logs"
...
output_dir="${OUTPUT_BASE}/${TASK_SUITE}/${timestamp}"
mkdir -p "$output_dir"

BATCH_SCRIPT=$(mktemp /tmp/fastwam_eval_XXXX.sh)
...
POLICY="POLICY_PATH_PLACEHOLDER"
...
job_name="fastwam-libero-${TASK_SUITE}"
sed -i \
